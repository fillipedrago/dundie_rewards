...
def add_person(db, pk, data):
    ...
    table = db["people"]
    person = table.get(pk, {})  # dict pegando o e-mail chamado
    created = not bool(person)  # se retornar dict preenchido, retorna False
    person.update(data)
    table[pk] = person
    if created:
        ...
        # TODO: Enctrypt and send only link not password
